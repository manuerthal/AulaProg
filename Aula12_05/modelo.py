...
class Medico(Pessoa):
    ...
    # método para expressar o medico em forma de texto
    def __str__(self):
        return f'{super().__str__()}, CRM: {self.crm}'
    # expressao da classe no formato json
    def json(self):
        json1 = super().json()
        json1.update({"crm":self.crm})
        return json1
...
class ExameRealizado(db.Model):
    ...
    # medico que fez o exame; não pode ser nulo (composição!)
    ...
    def __str__(self): # expressão da classe em forma de texto
        return f"{self.data}, {self.resultado}, " + \
            f"{self.pessoa}, {self.medico}, {self.exame}" 
...
if __name__ == "__main__":
    ...
    m1 = Medico(nome= 'José', email = "jose@gmail.com", telefone="123 456", crm="1245")
    db.session.add(m1)
    db.session.commit()
    print(m1)
    ...
    e1 = ExameRealizado(data="02/02/2020", exame=b12, 
        resultado="219,0 pg/mL", pessoa=p1, medico=m1)
    db.session.add(e1)
    db.session.commit()
