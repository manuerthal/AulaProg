...
@app.route("/listar_respirador")
def listar_respirador():
    # obter as respirador do cadastro
    respirador = db.session.query(Respirador).all()
    # aplicar o método json que a classe Respirador possui a cada elemento da lista
    respirador_em_json = [ x.json() for x in respirador ]
    # converter a lista do python para json
    resposta = jsonify(respirador_em_json)
    # PERMITIR resposta para outras pedidos oriundos de outras tecnologias
    resposta.headers.add("Access-Control-Allow-Origin", "*")
    return resposta # retornar...

# teste da rota: curl -d '{"nome":"James Kirk", "telefone":"92212-1212", "email":"jakirk@gmail.com"}' -X POST -H "Content-Type:application/json" localhost:5000/incluir_respirador
@app.route("/incluir_respirador", methods=['post'])
def incluir_respirador():
    # preparar uma resposta otimista
    resposta = jsonify({"resultado": "ok", "detalhes": "ok"})
    # receber as informações da nova respirador
    dados = request.get_json() #(force=True) dispensa Content-Type na requisição
    try: # tentar executar a operação
      nova = Respirador(**dados) # criar a nova respirador
      db.session.add(nova) # adicionar no BD
      db.session.commit() # efetivar a operação de gravação
    except Exception as e: # em caso de erro...
      # informar mensagem de erro
